...
function addTransactionDOM(transaction) {
  ...
  if (transaction.isStrikedOut === true) {
    item.classList.add("strikeout");
  }
  item.innerHTML = `
${transaction.text} <span>${sign}${Math.abs(transaction.amount)}</span>
  <button class="delete-btn" onclick="strikeOutTransaction(${
    transaction.id
  })">X</button>
  `;
  item.value = transaction.id;
...
function removeTransactionForStrikeOut(id) {
  transactions = transactions.filter((value) => value.id !== id);
}
...
function strikeOutTransaction(id) {
  let node;
  ...
  node.classList.add("strikeout");

  removeTransactionForStrikeOut(id);
  updateValues();
  updateLocalStorage();
  //`<li><del>${data.text} <span>${data.amount}</span></del></li>`;
}
